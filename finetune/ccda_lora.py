...
X_train = pd.read_csv('CcdA_seed.csv').sample(frac=1).reset_index(drop=True)
X_train['Sequence'] = X_train['Sequence'].str.slice(stop=1000)  # Limit sequences to 1000 characters

# Load the test data
X_test = pd.read_csv('CcdA_seed.csv').sample(frac=1).reset_index(drop=True)
...
def train_save():
    ...
    tokenizer.save_pretrained('ccda_model')
    peft_model.save_pretrained('ccda_model')
train_save()
