...
training_args = TrainingArguments(
    output_dir='./tadA',  # Where to save the model checkpoints
...
X_train = pd.read_csv('tada_blast_20.csv').sample(frac=1).reset_index(drop=True)
X_train['Sequence'] = X_train['Sequence'].str.slice(stop=1000)  # Limit sequences to 1000 characters

# Load the test data
X_test = pd.read_csv('tada_blast_20.csv').sample(frac=1).reset_index(drop=True)
...
def train_save():
    ...
    tokenizer.save_pretrained('tadA_VN_blast20model')
    peft_model.save_pretrained('tadA_VN_blast20model')
train_save()
