...
ADAPTER_PATH = "/mnt/hdd/chenzh/tadA_VN_blast20model/"
ESM_MAX_LEN  = 1022
...
# —— 初始化与注入（关键）
USE_TARGET_SEEDS   = True       # 初代从 TARGET_FASTAS 突变 20%（≈0.8 identity）
INJECT_EVERY       = 20           # 每 N 代注入一次
INJECT_FRAC        = 0.25         # 注入覆盖最差的比例
GUIDED_TARGET_ID   = 0.60         # 注入个体目标 identity
GUIDED_JITTER      = 0.05         # 注入时围绕 0.8 ± 抖动，避免同质化

# —— cos 质心数据
GREEN_FASTA  = "tada_uniport_blast_20.fasta"
RED_FASTA    = "tadA_VN.fasta"
TARGET_FASTAS= [(GREEN_FASTA, 1.0), (RED_FASTA, 0.0)]

# —— 批大小 / 变异率 / 代数
PROB_BS  = 32
EMB_BS   = 16
MUT_RATE = 0.20
origin   = 'tada_long_pool.csv'
ec_pick  = 4
epoch    = 100
...
def check_disorder_in_sequence(sequence, sequence_id):
    if not segmasker_available():
        ...
        return False
    fa  = f"temp_seqtada0_{sequence_id}.fasta"
    itv = f"temp_seqtada0_{sequence_id}.interval"
